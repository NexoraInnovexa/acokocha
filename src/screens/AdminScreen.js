...
export default function AdminDashboard() {
  ...
  useEffect(() => {
  // Subscribe to changes in sell_requests table
  const subscription = supabase
    .channel('public:sell_requests')
    .on(
      'postgres_changes',
      ...
      (payload) => {
        ...
        setRequests(prev => {
          switch (payload.eventType) {
            ...
            case 'DELETE':
              return prev.filter(r => r.id !== oldRequest.id);
            default:
              return prev;
          }
        });
...
const updateCar = async (id, updatedData) => {
  try {
    ...
    // Delete only images that were removed/replaced
    ...
    if (imagesToDelete.length > 0) {
      ...
      if (fileNames.length > 0) {
        ...
        if (storageErr) console.warn('⚠️ Failed to delete replaced images from storage:', storageErr);
      }
    }
...
const deleteCar = async (id) => {
 ...
 const markSold = async (id) => {
  try {
    ...
    // Refresh local state
    setCars(prev => prev.map(car => (car.id === id ? { ...car, sold: true } : car)));
    setFilteredCars(prev => prev.map(car => (car.id === id ? { ...car, sold: true } : car)));
    
  } catch (err) {
    console.error('Mark sold error:', err);
    Alert.alert('Error', 'Failed to mark car as sold.');
  }
};
...
const handleAccept = async (id) => {
  try {
    ...
    Alert.alert('Accepted ✅', 'The request has been accepted.');
  } catch (err) {
    console.error(err);
...
const handleComplete = async (id) => {
  try {
    ...
    Alert.alert('Completed ✅', 'The request has been marked complete.');
  } catch (err) {
    console.error(err);
...
// Terminate (delete with confirmation)
const handleTerminate = (id) => handleDelete(id);
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#000' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <Button
          ...
          style={{ marginBottom: 12, borderColor: colors.accent }}
 ...
 <View style={{ paddingHorizontal: 12, marginVertical: 8 }}>
      ...
      >
        ...
        {sellRequests.map(req => (
          <Card key={req.id} style={styles.card}>
           <Card.Content>
              <Text style={{ color: '#fff' }}>User: {req.name}</Text>
              <Text style={{ color: '#fff' }}>Phone: {req.phone}</Text>
              <Text style={{ color: '#fff' }}>Make: {req.make}</Text>
              <Text style={{ color: '#fff' }}>Model: {req.model}</Text>
              <Text style={{ color: '#fff' }}>Year: {req.year}</Text>
              <Text style={{ color: '#fff' }}>Mileage: {req.mileage}</Text>
              <Text style={{ color: '#fff' }}>Condition: {req.condition}</Text>
              <Text style={{ color: '#fff' }}>Created: {req.created_at}</Text>
              ...
              <ScrollView horizontal style={{ marginVertical: 8 }}>
                {req.images?.map((imgUrl, idx) => (
                  <Image
                    key={idx}
                    source={{ uri: imgUrl }}
                    style={{ width: 100, height: 100, marginRight: 8, borderRadius: 6 }}
                  />
                 ))}
                </ScrollView>
  ...
  {req.status === 'pending' && (
    <>
      <Button mode="outlined" style={{ marginRight: 8 }} onPress={() => handleAccept(req.id)}>Accept</Button>
      <Button mode="outlined" onPress={() => handleReject(req.id)}>Reject</Button>
    </>
  )}

  {req.status === 'rejected' && (
    <Button mode="outlined" onPress={() => handleDelete(req.id)}>Delete</Button>
  )}

  {req.status === 'accepted' && (
    <>
      <Button mode="outlined" style={{ marginRight: 8 }} onPress={() => handleComplete(req.id)}>Mark Complete</Button>
      <Button mode="outlined" onPress={() => handleTerminate(req.id)}>Terminate</Button>
    </>
  )}

  {req.status === 'completed' && (
    <Text style={{ color: '#0f0', fontWeight: 'bold' }}>Completed</Text>
  )}
</View>
